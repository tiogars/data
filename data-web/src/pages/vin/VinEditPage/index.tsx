...
import { useGetVinQuery, useUpdateVinMutation } from '../../../services/vinApi';
import type { VinEditPageProps } from './VinEditPage.types';

const emptyValues: VinFormValues = {
  appellationId: '',
  couleurId: '',
  typeVinId: '',
  maisonId: '',
...
export const VinEditPage: FC<VinEditPageProps> = ({ id }) => {
  ...
  useEffect(() => {
    ...
    reset({
      ...
      typeVinId: data.typeVinId ?? '',
      maisonId: data.maisonId ?? '',
      vinNomId: data.vinNomId ?? '',
      contenantId: data.contenantId ?? '',
      annee: data.annee?.toString() ?? '',
      degorgementMois: data.degorgementMois?.toString() ?? '',
      degorgementAnnee: data.degorgementAnnee?.toString() ?? '',
      dosageGrammesParLitre: data.dosageGrammesParLitre?.toString() ?? '',
      commune: data.commune ?? '',
      region: data.region ?? '',
      commentaires: data.commentaires ?? '',
      accordsMetsVins: data.accordsMetsVins ?? '',
      cepages: (data.cepages ?? []).map((c) => ({ cepageId: c.cepageId ?? '', pourcentage: c.pourcentage?.toString() ?? '' })),
      circonstanceIds: data.circonstances ?? [],
      tagIds: data.tags ?? [],
    });
  }, [data, reset]);
